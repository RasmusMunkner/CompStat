...
lr <- polynomial_schedule(1, 0.1, later = 300, p = 20) # Rapid decay after 300
...
p1 <- plot(trace) + ggplot2::ggtitle("Parameter Values")

p2 <- plot(design, trace %>% tail(1), post_transform = function(x) exp(x) / (1 + exp(x))) +
  ggplot2::lims(x = c(34,42), y = c(0,1)) +
  ggplot2::labs(x = "Temperature", y = "P(Dead|Temperature)") +
  ggplot2::ggtitle("Final Fitted Regression")

p3 <- ggplot2::ggplot(horses, ggplot2::aes(x = Temperature, y = dead)) +
...
## Profiling without tracing {.smaller}

```{r Profiling r SGD without tracing}
#| cache: true

profvis::profvis({
  trace <- SGD(
  opt_target_horses, adam, init_param = rep(0,opt_target$n_param),
  stop_crit = 400,
  batch_size = nrow(horses)/4,
...
trace <- SGD(
  opt_target_horses, adam, init_param = rep(0,opt_target$n_param),
  stop_crit = 400,
  batch_size = floor(nrow(horses)/4),
  trace_precision = "none"
  )
...
targets <- 8:16 %>% purrr::map(.f = function(i){
  x_spline <- x[1:2^i] %>% ExpandBspline()
  x_poly <- x[1:2^i] %>% ExpandPoly()
  list(
    x_spline = x_spline,
    x_poly = x_poly,
    lll_spline = logistic_loglikelihood(y[1:2^i], x_spline, lambda = 0),
    lll_poly = logistic_loglikelihood(y[1:2^i], x_poly, lambda = 0)
  )
})
...
lr <- polynomial_schedule(1, 0.9, later = 500)
trace <- SGD_CPP(targets[[1]]$lll_spline, seed = 0, stop_crit = 1e4, batch_size = 100)
