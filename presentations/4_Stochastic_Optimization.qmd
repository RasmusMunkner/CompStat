...
## A matrix derivation of the relevant likelihood
...
And
$$
g(p) = -\frac{1}{N}e^T(y \log(p) + (1-y)\log(1-p))
$$

Thus
$$
D_pg(p) = -\frac{1}{N}e^T\cdot\text{diag}\left(\frac{y}{p} + \frac{1-y}{1-p}\right)
$$
...
```{r Profiling Full R Implementation}
