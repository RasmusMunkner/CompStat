...
```{r Envelope with many tangent points, fig.width=7, fig.height=3, eval=F}
...
```{r Adaptive Sampler Benchmark, cache = T, eval=F}
z <- 8:16
x <- 2^z
...
```{r Autoplot Adaptation Benchmark, fig.width=7, fig.height=3, eval=F}
adaptive_sampler_bm %>% autoplot()
```

```{r Plot Adaptation Benchmark, fig.width=7, fig.height=3, eval=F}
...
```{r Defining an envelope and fitting it, eval=F}
set.seed(0)
...
```{r Simulating from the fitted envelope and investigate the fit, eval = F}
set.seed(0)
